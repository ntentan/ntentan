...
class oracle extends SQLDatabaseModel
{
	...
	protected function _getModelData($params=null,$mode=model::MODE_ASSOC, $explicit_relations=false,$resolve=true)
	{
		...
		else
		{
			...
			$query = sprintf("SELECT $field_list FROM %s ",$this->database).($do_join?$joins:"").($conditions!=null?" WHERE ".$conditions:"").$sorting;
	...
	public function formatField($field,$value,$alias = true)
	{		
		...
		switch($field["type"])
		{
			case "date":
			case "datetime":
				$ret =  "INITCAP(TO_CHAR(TO_DATE('19700101000000','YYYYMMDDHH24MISS') + NUMTODSINTERVAL($value, 'SECOND'),'DAY, DDTH MONTHYYYY'))";// as \"{$field["name"]}\"";
				break;
			case "enum":
