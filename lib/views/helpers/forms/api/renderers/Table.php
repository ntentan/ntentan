...
class Table extends Renderer
{
	...
	$ret = "<tr " . ($element->id()==""?"":"id='".$element->id()."_wrapper'") . " " . $element->getAttributes(Element::SCOPE_WRAPPER) . " >";
        if(!$element->isContainer && $element->renderLabel)
        {
            $ret .= "<td class='fapi-layout-table-label'>" . $this->renderLabel($element) . "</td>";		
        }
        ...
        $ret .= "<div class='fapi-message' id='".$element->id()."-fapi-message'></div>";
        if($element->hasError())
        {
