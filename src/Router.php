...
class Router
{
     ...
     * 
     */
    private static $attemptedControllers = [];
    ...
    public static function execute($route)
    {
        ...
        $staticParameters = self::getStaticRouteParameters($route, $staticRouteName);
        if(self::loadResource($staticParameters, $staticRouteName)) return;
        
        // Fallback on dynamic routes which are generated by callables if
        // static routes fail
        $dynamicParameters = self::getDynamicRouteParameters($route, $dynamicRouteName);
        if(self::loadResource($dynamicParameters, $dynamicRouteName)) return;
    ...
    private static function getDynamicRouteParameters($route, &$routeName)
    {
        ...
        if(empty($routeParameters)) {
            foreach(self::$dynamicRoutes as $routeName) {
                $parameters = call_user_func(self::$routes[$routeName]['function'], $route);
    ...
    private static function loadController($params = [])
    {
        ...
        if($resolvedControllerClass) {
            ...
            self::$attemptedControllers[] = $controllerClassName;
            return false;
        }
