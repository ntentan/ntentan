...
- Added `ntentan/kaikai` dependency for cache backend support.
- Added dedicated test cases for individual HTTP filters (`HeaderTest`, `MimeHeaderTest`, `RouteTest`, `GetTest`, `PostTest`).
...
- Cleaned up `Context` constructor and removed session management from application context.

Removed
-------
- Removed the session subsystem (`src/sessions/` including `SessionStore`, `PhpSessionStore`, `DefaultSessionContainer`, and `FileSessionHandler`).
- Removed `Method` filter in favor of `Route` method attributes (`Get`, `Post`).
